...
function DonutChart({ segments }) {
  ...
  const cx = 130, cy = 78, r = 62, inner = 38
  let angle = -Math.PI / 2
  ...
  return (
    <svg viewBox="0 0 260 185" style={{ width: '100%', height: 185 }}>
      {arcs.map((a, i) => <path key={i} d={a.path} fill={a.color} />)}
      <text x={cx} y={cy + 5} textAnchor="middle" fontSize="15" fontWeight="600" fill="#1a1a2e">{total}</text>
      <text x={cx} y={cy + 19} textAnchor="middle" fontSize="9" fill="#999990">total recalls</text>
      {/* Legend underneath — 2x2 grid */}
      {arcs.map((a, i) => {
        const col = i % 2
        const row = Math.floor(i / 2)
        const lx = (cx - legendItemW) + col * legendItemW + 2
        const ly = legendY + row * 26
        return (
          <g key={i}>
            <rect x={lx} y={ly} width={10} height={10} rx="2" fill={a.color} />
            <text x={lx + 14} y={ly + 8} fontSize="9" fontWeight="500" fill="#333">{a.label}</text>
            <text x={lx + 14} y={ly + 19} fontSize="9" fill="#999990">{a.pct}%</text>
          </g>
        )
      })}
    </svg>
...
function catStyle(cat) {
  const map = {
    food:     { background: '#F59E0B', color: '#fff' },
    products: { background: '#3B82F6', color: '#fff' },
    vehicles: { background: '#10B981', color: '#fff' },
    drugs:    { background: '#8B5CF6', color: '#fff' },
    devices:  { background: '#EF4444', color: '#fff' },
  }
  return map[cat] || { background: C.bgSoft, color: C.textMid }
}

function severityStyle(sev) {
  if (sev === 'Class I')   return { background: '#DC2626', color: '#fff' }
  if (sev === 'Class II')  return { background: '#D97706', color: '#fff' }
  if (sev === 'Class III') return { background: '#2563EB', color: '#fff' }
...
function RecallCard({ recall, onSelect }) {
  ...
  const topColors = { food: '#F59E0B', products: '#3B82F6', vehicles: '#10B981', drugs: '#8B5CF6', devices: '#EF4444' }
  const topColor = topColors[recall.category] || C.red
  ...
  return (
    <div
      ...
      style={{
        borderRadius: 12, padding: 16, cursor: 'pointer', background: C.bgCard,
        border: `1px solid ${hovered ? topColor + '60' : C.border}`,
    ...
    >
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 10 }}>
        <Badge style={catStyle(recall.category)}>{catLabel}</Badge>
        <Badge style={severityStyle(recall.severity)}>{recall.severity}</Badge>
        <Badge style={agencyStyle()}>{recall.agency}</Badge>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderTop: `0.5px solid ${C.border}`, paddingTop: 8 }}>
        ...
        <span style={{ fontSize: 11, color: topColor, fontFamily: MONO, fontWeight: 600 }}>View →</span>
      </div>
    </div>
  )
}
...
function Nav({ page, navigate }) {
  ...
  // Nav icons as inline SVG paths
  const NavIcon = ({ name }) => {
    const icons = {
      home: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M2 6.5L8 2l6 4.5V14a1 1 0 01-1 1H3a1 1 0 01-1-1V6.5z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/><path d="M6 15v-5h4v5" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/></svg>,
      food: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M3 2v4c0 1.66 1.34 3 3 3v5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/><path d="M3 2h6" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/><path d="M10 2c0 0 3 2 3 5s-3 5-3 5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>,
      products: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="5" height="5" rx="1" stroke="currentColor" strokeWidth="1.4"/><rect x="9" y="2" width="5" height="5" rx="1" stroke="currentColor" strokeWidth="1.4"/><rect x="2" y="9" width="5" height="5" rx="1" stroke="currentColor" strokeWidth="1.4"/><rect x="9" y="9" width="5" height="5" rx="1" stroke="currentColor" strokeWidth="1.4"/></svg>,
      vehicles: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M2 9l1.5-4h9L14 9v3H2V9z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/><circle cx="4.5" cy="12" r="1.5" stroke="currentColor" strokeWidth="1.2"/><circle cx="11.5" cy="12" r="1.5" stroke="currentColor" strokeWidth="1.2"/><path d="M4.5 7.5h7" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>,
      drugs: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M9.5 2.5l4 4-7 7-4-4 7-7z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/><path d="M6 6l4 4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>,
      devices: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><path d="M8 2v3M8 11v3M2 8h3M11 8h3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><circle cx="8" cy="8" r="3" stroke="currentColor" strokeWidth="1.4"/></svg>,
      guides: <svg width="14" height="14" viewBox="0 0 16 16" fill="none"><rect x="2" y="2" width="12" height="12" rx="2" stroke="currentColor" strokeWidth="1.4"/><path d="M5 6h6M5 8.5h4M5 11h3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>,
    }
    return icons[name] || null
  }

  const links = [
    { label: 'All Recalls', page: 'home', icon: 'home', hoverColor: '#EF4444' },
    { label: 'Food', page: 'food', icon: 'food', hoverColor: '#F59E0B' },
    { label: 'Products', page: 'products', icon: 'products', hoverColor: '#3B82F6' },
    { label: 'Vehicles', page: 'vehicles', icon: 'vehicles', hoverColor: '#10B981' },
    { label: 'Drugs', page: 'drugs', icon: 'drugs', hoverColor: '#8B5CF6' },
    { label: 'Devices', page: 'devices', icon: 'devices', hoverColor: '#EF4444' },
    { label: 'Guides', page: 'guides', icon: 'guides', hoverColor: '#6B7280' },
  ]
  const isActive = (p) => p === 'home' ? (page === 'home' || page === '') : page === p

  return (
    <nav style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '10px 20px', borderBottom: `0.5px solid ${C.border}`, flexWrap: 'wrap', background: C.bg, position: 'sticky', top: 0, zIndex: 100 }}>
      <div
        onClick={() => navigate('home')}
        style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 17, fontWeight: 700, color: C.navy, cursor: 'pointer', whiteSpace: 'nowrap', marginRight: 12 }}
      >
      ...
      <div style={{ display: 'flex', gap: 2, flex: 1, flexWrap: 'wrap' }}>
        {links.map(l => {
          ...
          const hovered = hoveredNav === l.page
          const accent = l.hoverColor
          return (
            <button
              key={l.page}
              onClick={() => navigate(l.page)}
              onMouseEnter={() => setHoveredNav(l.page)}
              onMouseLeave={() => setHoveredNav(null)}
              style={{
                display: 'flex', alignItems: 'center', gap: 5,
                fontSize: 13, cursor: 'pointer', border: 'none', fontFamily: FONT,
                padding: '6px 10px', borderRadius: 8,
                background: active ? accent + '18' : hovered ? accent + '12' : 'transparent',
                color: active ? accent : hovered ? accent : C.textMid,
                fontWeight: active ? 600 : 400,
                transition: 'all 0.15s ease',
              }}
            >
              <span style={{ color: active ? accent : hovered ? accent : C.textMuted, display: 'flex', alignItems: 'center' }}>
                <NavIcon name={l.icon} />
              </span>
              {l.label}
            </button>
...
function CatCardHover({ children, color, bg, onClick }) {
  const [hovered, setHovered] = useState(false)
  return (
    <div
      onClick={onClick}
      ...
      style={{
        ...
        background: hovered ? bg : C.bgCard,
        boxShadow: hovered ? `0 6px 20px ${color}20` : '0 1px 3px rgba(0,0,0,0.04)',
        transform: hovered ? 'translateY(-2px)' : 'translateY(0)',
        transition: 'all 0.2s ease',
      }}
    >{children}</div>
      ...
      <div style={{ padding: '20px 24px', borderBottom: `0.5px solid ${C.border}` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,minmax(0,1fr))', gap: 10 }}>
          {cats.map(cat => {
            const CatIcon = () => {
              const icons = {
                food: <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M4 3v5c0 2.2 1.8 4 4 4v5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/><path d="M4 3h7" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/><path d="M13 3s4 2.5 4 7-4 7-4 7" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>,
                products: <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><rect x="2" y="2" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.6"/><rect x="12" y="2" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.6"/><rect x="2" y="12" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.6"/><rect x="12" y="12" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.6"/></svg>,
                vehicles: <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M2 12l2.5-6h11L18 12v4H2v-4z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round"/><circle cx="6" cy="16" r="2" stroke="currentColor" strokeWidth="1.5"/><circle cx="14" cy="16" r="2" stroke="currentColor" strokeWidth="1.5"/><path d="M6 10h8" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>,
                drugs: <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M12 3l5 5-9 9-5-5 9-9z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round"/><path d="M7.5 7.5l5 5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/></svg>,
                devices: <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M10 2v4M10 14v4M2 10h4M14 10h4" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round"/><circle cx="10" cy="10" r="4" stroke="currentColor" strokeWidth="1.6"/></svg>,
              }
              return icons[cat.key] || null
            }
            return (
              <CatCardHover key={cat.key} color={cat.color} bg={cat.bg} onClick={() => navigate(cat.key)}>
                <div style={{ width: 38, height: 38, borderRadius: 10, background: cat.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 10, color: cat.color }}>
                  <CatIcon />
                </div>
                <div style={{ fontSize: 13, fontWeight: 600, color: C.text, lineHeight: 1.3 }}>{cat.label}</div>
                <div style={{ fontSize: 11, color: C.textMuted, marginTop: 3 }}>{cat.count} active recalls</div>
                <div style={{ fontSize: 11, color: cat.color, marginTop: 8, fontWeight: 500 }}>View all →</div>
              </CatCardHover>
            )
          })}
        </div>
